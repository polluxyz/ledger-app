...
> 狀態：**送審中**（2026-09-23），與 `tasks/phase-2h-todo.md` 一起送審。
...
⚠️ **第二項會改 3 行 e2e 的期望字串**（`transactions.spec.ts:62, 72, 90`：`'$-120'` → `'-$120'` 等）。這是 spec §2 假設 6，**必須先取得同意**。斷言意圖不變。
...
- **< 900px 新增表單收合**：`HomePage` 加一個 `formExpanded` state（預設 `false`），加一顆「新增一筆交易」展開鈕。但 **≥ 900px 用 CSS 強制展開並隱藏那顆鈕**——桌機上表單永遠看得到，e2e（1280px）不受影響。
...
| 步驟 | 內容                                                                                 | 誰     | 相依 |
| ---- | ------------------------------------------------------------------------------------ | ------ | ---- |
| A    | PR-A：三個顯示錯誤（需先同意 3 行 e2e）                                              | 協調者 | 無   |
...
| 驗什麼       | 怎麼驗                                                                         |
| ------------ | ------------------------------------------------------------------------------ |
| SC-24.1–2    | 單元測試（收合記憶）＋ 截圖（1440、1280、1024、900、390）                      |
...
基準線：web 單元測試 **33 檔**（條數在步驟 0 實測）、api **9 套 119 條**、e2e **20 條**。
