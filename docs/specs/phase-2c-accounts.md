...
| **解除封存帳本**                   | 後端目前只有 `POST /ledgers/{id}/archive`，封存後無法復原。前端因此在封存前要求打字輸入帳本名稱（Slice 2 D3）。實作很小：`archivedAt` 設回 `null` + 一條 e2e                                                                            | 待定                                   |
| **封存帳本仍可退出**               | `LedgerAccessGuard` 對封存帳本擋掉所有非 GET 請求，`DELETE …/members/{userId}` 也在內——成員因此再也退不出去（Slice 2 S6-D4）。封存的用意是「收起來」，不該連「我不想再看到這本帳」都做不到。修法是在 guard 開一個窄例外：僅允許移除自己 | **與上一列一起做**；Slice 2 收尾時評估 |
