...
**權限模型決策（2026-08-07 定案）**：

- **Owner 對稱模型**：所有 OWNER 平等，任一 OWNER 可加入/升降/移除任何人（含其他 OWNER），唯一鐵律是帳本至少留一名 OWNER。不採「不可動其他 owner」（會造成治理死結，且 owner 本可刪整本帳，該規則擋不住帳號被盜）。加入成員時允許直接指定 OWNER。
- **移除的雙重授權**：本人可退出自己；移除他人須為 OWNER，否則 `403`（`CANNOT_MANAGE_OTHER_MEMBER`）。
- **交易編輯採共享帳本模型**（Step 6 實作）：帳本內任何 EDITOR 可編輯/刪除**任何**交易，不以 `creatorId` 限制；`creatorId` 僅供記錄與顯示。
- **帳號安全防護**（敏感操作通知、稽核日誌、軟刪除復原、2FA）屬未來安全強化，見 §9。
...
| 帳號安全防護   | 對付帳號被盜的正解不是權限規則（owner 本可刪整本帳），而是：敏感操作（改成員/刪帳本）email 通知、操作稽核日誌、成員/交易軟刪除＋復原視窗、2FA。獨立的安全強化主題，未來排期                                                                                         |
