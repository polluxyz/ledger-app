...
>
> **不新增任何 npm 套件、不動 Prisma、不動 API、不動 CI。** 唯一要改既有 e2e 的是 PR-A 的 3 行期望字串（spec §2 假設 6，2026-09-23 已同意）。
>
> 通用驗收（每個任務皆適用，不再重複）：
...
A.4 會改 3 行 e2e，已於 2026-09-23 取得同意（spec §2 假設 6）。
...
- [x] **2.2 W2 首頁工作台**
  - 檔案：plan §8 的 W2 清單。
  - 內容：三欄版面、右側面板（D8、D9）、帳戶餘額移進面板、窄螢幕編輯時隱藏列表（D10，只求不壞）、焦點回歸、統計卡換樣式。
  - 驗收：
...
- [x] **3.1 新增 `e2e/layout.spec.ts`**
  - 內容：建 21 筆交易，驗 1440×900 可見 ≥ 10 筆（SC-24.3 改以 1440×900 驗收）、篩選欄位同高、五個頁面的頁首左緣相同、375px 沒有橫向捲動；另外驗「`localStorage` 設成淺色後重新整理，第一次繪製前 `<html>` 就帶 `data-theme="light"`」（SC-29.3）。
  - 驗收：新測試綠。
...
- [x] **3.3 截圖驗收**
  - 內容：900／1024／1280／1440／2560 × 深淺兩色 × 側欄展開與收合；375／390 只驗「不壞」（spec SC-24.4）。
...
- [x] **3.7 修掉 `LedgerDetailPage.test.tsx:121` 的 race**
  - 原因（W1 回報）：側欄的使用者區塊也訂閱 `useCurrentUser`，讓 `/users/me` 在斷言前就解析完，於是「我的角色：可編輯」與 Bob 那一列的「可編輯」同時出現，`getByText('可編輯')` 對到兩個。這條測試原本是靠「我的角色還沒載入」這個 race 才過的。
  - 內容：只改選取方式（例如限定在成員列表內找，或等身分載入後改用 `getAllByText`），斷言意圖不變。
  - 驗收：連跑 3 次都綠。
