...
#### 補充討論：轉帳能不能靠型別限制？（2026-08-27）

開發者提出一個支持帳戶型別的新理由：「轉帳的轉出跟轉入應該只會有銀行帳戶跟現金，
有類別就可以很好地去設定。」

**這個理由不成立。** 還信用卡就是一筆轉帳（銀行 → 信用卡），而且是最常見的轉帳之一。
用信用卡預借現金則是反方向（信用卡 → 現金），罕見但存在。**信用卡必須能當轉入帳戶，
也必須能當轉出帳戶。** 若真的照這個想法用型別去限制轉帳的可選帳戶，會擋掉一個正常用途，
而使用者只會看到「我的信用卡不在選單裡」，不知道為什麼。

**但底下有一個更強的理由，值得記下來。** 開發者真正觀察到的是：**信用卡的「餘額」和
其他帳戶的「餘額」不是同一種東西。** 銀行的 8000 是資產，信用卡的 -3450 是負債。
目前兩者混在同一排卡片裡，用同一個 `$` 符號、同一種呈現方式。

型別若要做，正確的用途是這三個，都與「限制轉帳」無關：

1. 資產與負債在列表上分區顯示，而不是混排。
2. 未來算「淨資產」時知道哪些相加、哪些相減。
3. 下拉選單分組（銀行 > 國泰世華），也就是本節最上面原本的理由。

**結論：維持不做**，觸發條件不變。上面第 1、2 點正好對應既有觸發條件裡的
「資產 / 負債分開統計」——**目前還沒有任何畫面需要它**，統計卡片仍在等後端彙總端點。
等那個端點要做時，這一題會自然浮上來，屆時一併決定。

**若日後真的做了型別，這裡有一條界線**：型別是用來**呈現與彙總**的，
**不可以拿來限制轉帳的可選帳戶**。理由就是本節開頭那一段。
