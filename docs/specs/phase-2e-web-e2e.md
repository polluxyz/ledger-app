...
**第 7～9 條為 Slice 3 新增**（2026-08-25，`transactions.spec.ts`）。三條都圍著同一件事：改動交易之後帳戶餘額必須跟著變。餘額由後端算出，前端少做一次快取失效不會拋錯、不會讓任何單元測試變紅，畫面上的數字只會停在舊值——只有真的操作一遍才看得到。
...
| 交易的篩選與分頁不進 e2e         | 要驗第二頁得先造 21 筆前置資料，換到的資訊有限。元件測試已釘住送出的查詢字串               | 分頁行為出問題時 |
...
`transactions.spec.ts` 加入情境 7～9，全套共 **11 條**，本機一輪約 18 秒。
