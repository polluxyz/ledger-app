...
> 狀態：**3b-1 已實作；3b-1 修訂（決策 30 以此結清）與 3b-1 的 Web 畫面已實作；3b-2 未開始**（2026-09-24；開發者表示不完備處等實際成果出來再調整）
> 依據：`專案決策脈絡.md`「階段三定案」（2026-09-23），加上 2026-09-24 的假設清單確認，詳見 §2。
> 定位：**後端為主**。Web 只做讓既有畫面不壞的最小相容改動（§7），借還帳的畫面另立步驟。
> 前置：3a 好友系統（PR #54）已合併。
> 執行順序：3a（已完成）→ 3b-1 單邊借還（已完成）→ **3b-1 修訂「以此結清」＋ 3b-1 的 Web 畫面** → 3b-2 連動 → 3b-2 的 Web 畫面（含好友畫面）。代墊另開一輪設計，不在本步。
> 順序在 2026-09-24 調整過：3b-1 的 Web 畫面提前到 3b-2 之前，理由見 §11。同日新增決策 30「以此結清」（§2.3）。
...
### 2.3 2026-09-24 第二次定案：以此結清

| #   | 決策                                                                                                                       | 理由摘要                                                                         |
| --- | -------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| 30  | 記還款時可以勾「**以此結清**」：金額可以少於或多於未清餘額，這筆還款之後債務就是 `SETTLED`，差額記在債務上，**不產生交易** | 借 93 還 90 就算了、還 95 多給了，是日常。帳戶照實際收付變動，差額留待統計時計入 |
| 31  | 借出與借入在統計上**不算**收入或支出（重申決策 22）                                                                        | 會計上借貸是資產與負債的轉換，不影響損益                                         |
| 32  | 完全免除記成損失：**暫不處理**，維持決策 8                                                                                 | 開發者 2026-09-24 決定先不考慮                                                   |

決策 30 的細則：

- 差額的正負號**從擁有者的角度**看：正數＝對我有利（借出時多收、借入時少付），負數＝對我不利。
- 差額是**算出來的**，不存欄位：已還總額與本金的差，再依方向決定正負。與決策 5 同理。
- 借入的一方也能用。這補上一個缺口：單邊記錄的債權人不是使用者，沒有人能按「免除」，借入方原本無法關掉「對方說少還幾塊沒關係」的債務。
- 一筆債務最多一筆**未刪除**的結清還款（資料庫部分唯一索引保證）。刪掉它，債務回到 `OPEN`，差額消失。
- 債務有結清還款時**不能改本金**（`409 DEBT_NOT_OPEN`）。要改，先刪掉結清的那筆還款。否則差額會跟著本金默默改變。
- 沒勾「以此結清」時，規則不變：超過未清餘額回 `409 DEBT_OVERPAYMENT`。
- 同一次修訂補一個缺口：記還款的 `record` 接受 `null`，表示**明確不產生交易**。原本省略 `record` 會沿用本金交易，本金有交易的債務就沒辦法記一筆「沒經過帳戶」的還款（例如在系統外收到的現金已經另外記過）。

### 2.4 兩條舊規則在決策 15 下的新解讀
...
| 狀態       | 條件                                 | 可以做的事                              |
| ---------- | ------------------------------------ | --------------------------------------- |
| `OPEN`     | 未免除，沒有結清還款，且未清餘額 > 0 | 記還款、改內容、免除、連動、刪除        |
| `SETTLED`  | 未免除，且有結清還款或未清餘額 = 0   | 刪除還款（會回到 `OPEN`）、改備註、刪除 |
| `FORGIVEN` | 已免除                               | 改備註、刪除                            |

還款金額不得超過當下的未清餘額，超過回 `409 DEBT_OVERPAYMENT`。同理，把本金改得比已還總額還小（自己改，或接受對方的 `AMEND` 提議）也回 `409 DEBT_OVERPAYMENT`。

**例外是結清還款（決策 30）**：勾了「以此結清」的還款，金額可以多於未清餘額。以結清還款結清的債務：

- 未清餘額回傳 `0`，不是「本金減已還」算出來的負數或正數。
- 差額另外回傳在 `settlementDifference`（§5.5）。
- 不能改本金，回 `409 DEBT_NOT_OPEN`；要改先刪掉結清的那筆還款。
...
| ---------------------- | --------- | ----------------------------------------------------------------------------------------------------- |
| 記一筆還款             | `PAYMENT` | 加一筆還款；對方選帳本與帳戶，或不產生交易。發起者勾了「以此結清」時，對方那筆也是結清還款（決策 30） |
...
- 接受 `PAYMENT` 時若會讓對方那份超額還款，回 `409 DEBT_OVERPAYMENT`，提議維持 `PENDING`，對方可以改拒絕。結清的 `PAYMENT` 沒有這個限制。
...
model DebtPayment {
  ...
  transactionId String?   @unique
  /// 「以此結清」（決策 30）：這筆還款之後債務就是 SETTLED，金額可以不等於未清餘額。
...
4. `DebtPayment` 的部分唯一索引：同一個 `debtId`，`settles AND "deletedAt" IS NULL` 最多一筆（決策 30，3b-1 修訂時加）。
...
// POST /debts/{id}/payments
{
  ...
  date: string;
  note?: string;
  record?: { ledgerId: string; accountId?: string } | null;  // 省略 = 沿用本金交易的帳本與帳戶（本金沒有交易時 = 不產生交易）；null = 明確不產生交易（3b-1 修訂）
  settles?: boolean;              // 以此結清（決策 30）。true 時金額可以多於未清餘額
}
```
...
```ts
{
  ...
  outstanding: number,                       // 算出來的；以結清還款結清時為 0
  status: 'OPEN' | 'SETTLED' | 'FORGIVEN',
  settlementDifference: number | null,       // 決策 30。只有以結清還款結清時有值；正數＝對我有利
  transactionId: string | null,
  payments: Array<{ id, amount, date, note, transactionId, settles }>,
  link: null | {
...
`link.counterpart.outstanding` 與自己的 `outstanding` 不同時，就是兩邊不一致（§2.4）。這是對方那份唯一會回給我的內容。
...
- **SC-D23**：借出 93，記一筆 90 並勾結清 → 狀態 `SETTLED`、`outstanding` 為 0、`settlementDifference` 為 −3；帳戶收到 90（`COLLECT` 90）。
- **SC-D24**：借出 93，記一筆 95 並勾結清 → `SETTLED`、`settlementDifference` 為 +2、帳戶收到 95。不勾結清時同樣的 95 → `409 DEBT_OVERPAYMENT`。
- **SC-D25**：借入 93，還 90 並勾結清 → `SETTLED`、`settlementDifference` 為 +3、帳戶付出 90（`REPAY` 90）。
- **SC-D26**：刪除結清的那筆還款 → 回到 `OPEN`，`settlementDifference` 為 `null`，未清餘額照「本金減已還」算；對應的交易一起軟刪除。
- **SC-D27**：有結清還款時 `PATCH` 本金 → `409 DEBT_NOT_OPEN`，什麼都沒寫；改備註照常。
- **SC-D28**：同一筆債務已有未刪除的結清還款，再記一筆（勾或不勾結清）→ `409 DEBT_NOT_OPEN`；資料庫的部分唯一索引另外擋住並行寫入。
- **SC-D29**：剛好還清未清餘額且勾結清 → `SETTLED`、`settlementDifference` 為 0。每人淨額不計已結清的債務（與 SC-D11 一致）。
- **SC-D30**：本金有交易的債務，記還款時 `record: null` → 還款不產生交易、帳戶餘額不變；省略 `record` 仍沿用本金交易的帳本與帳戶。
...
| 呆帳是否計入損失     | 免除不產生交易，統計看不到這筆損失（2026-09-23 已接受的代價；2026-09-24 決策 32 重申） | 做統計報表時                    |
| 結清差額如何計入損益 | 決策 30 的 `settlementDifference` 已算得出來，報表要把它顯示成其他收入或其他損失       | 做統計報表時                    |
...
**3b-1 修訂：以此結清**（一個 PR，後端，決策 30）

1. shared 契約：`CreateDebtPaymentRequest.settles`、`DebtPayment.settles`、`Debt.settlementDifference`。
2. schema + migration：`DebtPayment.settles`，手寫部分唯一索引（§4.3 第 4 條）。
3. 狀態計算與記還款、刪還款、改本金的規則（SC-D23～SC-D30）；還款的 `record` 接受 `null`。

**3b-1 的 Web 畫面**（一個 PR，見 `phase-3b1-web.md`）
