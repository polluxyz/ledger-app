...
- **SC-7**：帳本列表 / 建立 / 切換；帳本明細顯示成員與角色。**建立時可選擇「私人 / 共享」與「是否與帳戶連動」**（兩者建立後皆不可變更，須於畫面上說明）。選共享時可就地輸入參與者的 email 一併加入；某位加不進去時帳本照建，畫面留著讓使用者修正重試（見 `phase-2d-ledger-kind.md` 與 `tasks/archive/phase-2b-slice-2-plan.md` D10）。
...
- **SC-19｜版面**：桌機取向的 dashboard 版面（見 `docs/specs/phase-2f-web-layout.md`）。
  第 1、2 項已由 2h 的 SC-24 取代（見下一節）。
  1. 1440px 寬時導覽在側邊欄，內容區不超過 70rem 且置中。
  2. 首頁在寬螢幕分兩欄；低於 900px 退回單欄。
  3. 375px 到 2560px 之間任何寬度都不出現橫向捲動。
  4. **既有 15 條 e2e 不需要修改斷言或選取器即全數通過**——版面重整最可能弄壞的
     就是「連結還在不在、切換器還找不找得到」，而那三件事 e2e 全部釘住了。

### 2h 帶進來的新條件（視覺改版）

- **SC-24～SC-30**：黑金視覺改版——三欄工作台、深淺色切換、往下展開的建立表單、
  圖示化的列操作。條文全部在 `docs/specs/phase-2h-web-visual.md` §3，這裡不重抄。
- **取代關係**：SC-19.1（導覽在側邊欄、內容不超過 70rem）與 SC-19.2（900px 以下單欄）
  由 SC-24 取代；SC-19.3（375–2560px 不橫向捲動）由 SC-24.6 沿用；SC-19.4
  （既有 e2e 不改）由 SC-27.1 延續。

### 2i 帶進來的新條件（版面第二輪）

- **SC-31～SC-37**：側欄開合動畫與 icon 不位移、使用者選單、頁首帳本切換器、首頁 dashboard 與交易分頁、記帳頁的右側欄、中間區置中。條文在 `docs/specs/phase-2i-web-layout-v2.md` §3。
- **取代關係**：SC-24.5（五頁標題左緣相同）由 SC-36.4（同一類頁面置中軸相同）取代；SC-24.3 與 SC-25.2～25.6 移到交易頁（SC-34）。
...
- [x] **5. Slice 2｜帳本 + 成員**：帳本列表 / 建立（含**私人 / 共享**與**連動設定**）/ 切換 / 明細、成員管理、**封存**（SC-7、SC-8、SC-16、SC-17）。
- [x] **6. Slice 3｜交易的完整生命週期**：編輯 / 刪除 / 分頁 / 篩選（SC-10）、**轉帳**（SC-15）。
- [x] **7. 2f｜版面重整 + 設計 token**：外殼改成 dashboard（頂列 + 側邊欄 + 內容區）、
      窄螢幕浮動選單、首頁分兩欄、`global.css` 的設計 token 擴充（SC-19）。
      見 `docs/specs/phase-2f-web-layout.md`。
- [x] **8. Slice 4｜分類管理 + 個人資料**（SC-9、SC-11）。決策見下方「Slice 4 的決策」。
- [x] **9. 收尾（2g）**：CSP、錯誤訊息在地化、分類排序、README 與元件測試（SC-12、SC-20～SC-23）。
...
- **版面重整插在 Slice 4 之前**（2026-08-26 決定）：Slice 4 要新增兩個頁面。
  先定骨架，那兩頁只做一次；反過來就是同樣的版面工做兩次。骨架本身的成本不隨
  時間變大，但被舊版面污染的頁面會愈積愈多。

### Slice 4 的決策（2026-09-23 拍板，實作完成）

D1～D6 由開發者在提案頁上選定，D7～D12 是實作計畫階段補上、一併核可的。
完整的理由與替代方案見 `tasks/archive/phase-2b-slice-4-plan.md` §4。

| #   | 結論                                                                                              |
| --- | ------------------------------------------------------------------------------------------------- |
| D1  | 分類做成獨立一頁 `/categories`，**頁內自己的帳本下拉**，與側邊欄的作用中帳本切換器各自獨立        |
| D2  | 支出與收入兩區塊上下並列，各自有新增按鈕；型別由按鈕帶入，表單上沒有型別欄位                      |
| D3  | 刪除分類用一般確認彈窗，不要求輸入名稱（被引用的分類後端已擋，刪得掉的一定沒有歷史資料）          |
| D4  | 非 EDITOR 隱藏新增／改名／刪除，並顯示一行說明。**這只決定按鈕畫不畫，授權的防線在後端**          |
| D5  | 錯誤訊息維持後端原文（英文），在地化留到收尾處理（見下方技術債表）                                |
| D6  | 個人資料入口放側邊欄，`/profile`；登出維持在側邊欄底部                                            |
| D7  | 頁內選的帳本 id 放網址查詢字串 `?ledgerId=`，用 `useSearchParams()`——重整保留、可加書籤、連得過來 |
| D8  | 頁內帳本 ≠ 作用中帳本時顯示提示條與一鍵切換。**這是 D1 選 C 的必要配套**，沒有它使用者會記錯帳本  |
| D9  | 下拉清單用 `useActiveLedger().ledgers`，不另外發請求；角色直接取自 `LedgerSummary.role`           |
| D10 | 帳本明細頁的「管理這本帳本的分類」連結帶 `?ledgerId=`，**不改作用中帳本**；封存帳本不顯示         |
| D11 | 網址的 `ledgerId` 對不上清單就退回作用中帳本並顯示說明，不導頁、不清空畫面                        |
| D12 | `CategoryDialog` 新增與改名共用一份表單，用 key 重建；改名時型別不可變                            |
...
| ~~CSP（內容安全政策）~~ 部分完成 | **2g 完成建置時注入的部分（2026-09-23）**。`script-src` 與 `style-src` 都不需要 `unsafe-inline`，實測過六頁零違規。⚠️ `frame-ancestors` 在 meta 裡無效，**防 iframe 嵌入尚未生效**，要等有靜態主機。見 `security-baseline.md` SEC-4 | ~~收尾~~ 剩餘部分綁在部署   |
| token 撤銷                       | 目前登出只是前端刪掉 token，舊 token 仍有效 7 天。屬後端變更，需另立 Step。見 `security-baseline.md` SEC-2                                                                                                                          | 上線前必達（本階段外）      |
| ~~錯誤訊息在地化~~ ✅            | **已完成（2026-09-23，2g）**。前端 `lib/error-messages.ts` 收錄 23 個 `errorCode` 的中文文案，查不到的代碼退回後端原文                                                                                                              | ~~收尾~~ 已完成             |
| `details` 欄位層級訊息在地化     | 驗證失敗時後端回的 `details` 是 class-validator 產生的英文字串（例如 `name must be longer than or equal to 1 characters`），前端原樣呈現。**在地化必須改後端**，不是前端對照表能解決的                                              | 階段三以後                  |
