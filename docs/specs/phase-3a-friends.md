...
| 9   | 被拒絕後，**7 天內不能對同一人重送**；發起者看得到「已拒絕」                       | 防止反覆騷擾。隱藏拒絕的話，發起者只會看到永遠的「待確認」並一直重送                                          |
...
- **email 大小寫不同會找不到人。** 現有註冊流程沒有把 email 轉成小寫，`Foo@x.com` 與 `foo@x.com` 被當成兩個人。本步沿用現況做精確比對。修正需要處理既有資料，另開 `fix/` 分支處理（見 §8）。
...
model FriendRequest {
  ...
  /// 接受、拒絕或取消的時間。決策 9 的 7 天冷卻期從這裡起算。
...
| 方法與路徑                           | 說明                                                 | 成功            | 錯誤                                                                                                                               |
| ------------------------------------ | ---------------------------------------------------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `POST /friend-requests`              | body `{ email }`                                     | `201`，回傳邀請 | `400 CANNOT_FRIEND_SELF`、`404 USER_NOT_FOUND`、`409 ALREADY_FRIENDS`、`409 FRIEND_REQUEST_PENDING`、`409 FRIEND_REQUEST_COOLDOWN` |
...
`CANNOT_FRIEND_SELF`、`ALREADY_FRIENDS`、`FRIEND_REQUEST_PENDING`、`FRIEND_REQUEST_COOLDOWN`、`FRIEND_REQUEST_NOT_PENDING`、`INVITE_LINK_INVALID`。`USER_NOT_FOUND` 沿用既有的。
...
- **SC-F2**：以未註冊的 email 送出邀請 → `404 USER_NOT_FOUND`，資料庫沒有新增任何一筆。
...
- **SC-F5**：B 拒絕 A 的邀請 → A 的 `outgoing` 清單看到 `DECLINED`；7 天內 A 重送 → `409 FRIEND_REQUEST_COOLDOWN`；超過 7 天 → 可以重送。
...
- **時間相關**（SC-F5 的 7 天、SC-F7 的 10 分鐘）：service 透過可注入的時鐘取得現在時間，測試裡直接指定，不真的等。
...
| 項目                   | 說明                                                         | 時機                           |
| ---------------------- | ------------------------------------------------------------ | ------------------------------ |
| **email 大小寫正規化** | 註冊與查詢都轉小寫，並處理既有的大小寫重複資料               | **另開 `fix/` 分支，建議儘早** |
...
| 封鎖                   | 目前沒有需求。解除好友 + 7 天冷卻期已涵蓋大部分騷擾情境      | 有實際騷擾回報時               |
