...
>
> **驗收紀錄（2026-08-26）**：開發者在瀏覽器實測後確認通過（Step 8.2）。
> 實測發現一個問題並已修掉：編輯彈窗被欄位撐寬 46px，要左右捲才看得到欄位右半邊。
...
| `test(web): cover editing, deleting and transfers…`       | Step 7（情境 7～9）            |
| `test(web): cover filtering, paging and cross-member…`    | Step 7（情境 10～12）          |
| `fix(web): stop form fields from overflowing the dialog…` | Step 8.2 實測發現的問題        |
...
| D11 | e2e 補 6 條（編號 7～12）——2026-08-25 修訂，原訂 3 條                                |
...
- [x] **0.1 開分支**
  ...
  - 驗收：`main` 未被動到。
- [x] **0.2 修正 `docs/specs/phase-2-web-mvp.md` §9 的三處過期記述**
...
- [x] **1.2 `useUpdateTransaction` / `useDeleteTransaction`**
...
- [x] **3.1 `TransactionForm` 收選填的 `transaction`**
...
- [x] **3.3 清空備註（D8）**
  - 內容：編輯模式的備註為空時送 `''`。
  - 驗收：元件測試比對 body。
- [x] **3.4 `TransactionDialog`**
  - 內容：把表單包進 `Dialog`，標題「編輯交易」。資料流（開哪一筆、關閉）留在 `HomePage`。
  - 驗收：元件測試：開啟後看得到預填值。

## Step 4：刪除

- [x] **4.1 `TransactionList` 加編輯 / 刪除入口**
...
- [x] **4.2 刪除確認與送出**
  ...
  - 驗收：元件測試：確認後該筆從列表消失。
- [x] **4.3 兩條快取失效測試**（原 Step 1.3，因為要走真實畫面操作才移到這裡）
...
- [x] **5.1 `useCategories` 的 `type` 改為可選（D5）**
  - 內容：`type?: CategoryType`；沒給就不帶 `?type=`。query key 帶 `type`。
  - 驗收：既有兩個呼叫點不受影響。
- [x] **5.2 `TransactionFilters` 元件**
...
- [x] **6.1 `components/Pagination.tsx`**
...
- [x] **7.1 六條 e2e 情境（D11，2026-08-25 修訂）**
  ...
  - 驗收：`pnpm --filter @ledger/web test:e2e` 共 15 條全綠（含彈窗溢出那條）。
- [x] **7.2 文件回寫**
  - 內容：2e spec §7 補六條情境、§8 更新缺口表、§12 補實作紀錄；`docs/specs/phase-2-web-mvp.md` §9 的 Step 6 打勾。
  - 驗收：文件與實作一致。
