...
| 7   | 編輯交易的金額 → 帳戶餘額跟著變                                          | SC-10        |
| 8   | 刪除交易 → 該筆從列表消失，餘額還原                                      | SC-10        |
| 9   | 轉帳 → 兩個帳戶的餘額都正確變動                                          | SC-15        |
| 10  | 篩選型別 → 只留下符合的交易；清除篩選後回到全部                          | SC-10        |
| 11  | 21 筆 → 翻到第 2 頁 → 改篩選就回到第 1 頁                                | SC-10        |
| 12  | 編輯別人記的交易 → 帳戶欄位鎖住，錢仍記在他的戶頭                        | SC-10、SC-18 |

第 6 條要先讓**帳號 B 在共享帳本記一筆交易**，否則後端不會回 409，那條分支根本走不到。這是六條裡唯一需要額外前置資料的。

第 3、4 條需要兩個瀏覽器 context（各自的 `localStorage`），Playwright 原生支援。

**第 7～12 條為 Slice 3 新增**（2026-08-25，`transactions.spec.ts`）。

第 7～9 條圍著同一件事：改動交易之後帳戶餘額必須跟著變。餘額由後端算出，前端少做一次快取失效不會拋錯、不會讓任何單元測試變紅，畫面上的數字只會停在舊值——只有真的操作一遍才看得到。

第 12 條最值得走真的後端：**帳戶遮蔽是後端做的**，元件測試把它 mock 掉了。只有 e2e 證得出「別人的帳戶真的被遮起來」，以及「不帶 `accountId` 送出時，那筆錢仍然記在他的戶頭」。它的斷言直接查乙的帳戶餘額，不只看畫面。

第 11 條要 21 筆前置資料，用 `createTransaction` 跑一個迴圈建立，整條測試 0.9 秒。
...
| 只涵蓋 Slice 2 與 Slice 3 的情境 | 先把地基做完；補其他 slice 是後續的小工。Slice 0（註冊登入）與 Slice 1（帳戶管理）仍未涵蓋 | 各 slice 收尾時  |
...
## 12. 後續 slice 的補充（2026-08-25，Slice 3）

`transactions.spec.ts` 加入情境 7～12，全套共 **14 條**，本機一輪約 19 秒。

> **本節推翻了 Slice 3 plan 的 D11。** 那條決定「篩選與分頁不進 e2e」，理由是
> 「造 21 筆前置資料成本不划算」。實際做下來，那個迴圈讓整條測試花 0.9 秒，
> 理由不成立，因此補上情境 10～12。
>
> 情境 12（編輯別人的交易）是其中最該做的一條：**帳戶遮蔽是後端做的**，元件測試
> 把它 mock 掉了。驗證方式是把 `TransactionForm` 的 `accountLocked` 改成
> `false` 再跑一次——測試變紅，確認它真的守得住。

實作時遇到、值得記著的兩件事：

1. **首頁多了篩選列之後，`page.getByLabel('分類')` 會同時對到兩個元素**（新增表單一個、篩選列一個），Playwright 的 strict mode 直接報錯。解法不是加 `data-testid`，而是先縮小範圍——新增表單的 fieldset 有 `<legend>新增一筆交易</legend>`，那就是它的無障礙名稱，`getByRole('group', { name: '新增一筆交易' })` 選得到。同一招也用在 Vitest 的元件測試。
   共用的定位輔助函式因此抽到 `e2e/ui.ts`（原本散在 `ledgers.spec.ts` 裡）。
2. **負餘額顯示成 `$-120` 而不是 `-$120`**（`AccountBalances` 是 `${formatAmount(balance)}`，而 `formatAmount(-120)` 已經帶著負號）。這是 Slice 1 就存在的行為，本次只是被 e2e 的斷言照出來。**沒有改**——那是版面決策，不屬本切片範圍。要改的話動 `formatAmount` 的呼叫端即可。

`data-testid` 依然是 0 個，產品程式碼沒有為了測試而改。
