...
| 錯誤訊息在地化             | 後端的錯誤訊息是英文（例如 `Cannot delete a category that transactions reference.`），前端一律原樣呈現，所以中文介面裡會冒出英文句子。**全站一致，不是單一頁面的問題。** Slice 4 刻意不處理（D5），避免範圍擴散到全站文案  | 收尾                        |
