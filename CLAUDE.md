...
**階段二 — 前端雛形（Web）**：在現有後端上建立 `apps/web`（React + TypeScript + Vite），跑通核心記帳的使用者介面。先切一條垂直薄片（註冊 / 登入 → 檢視一本帳本 → 新增 / 列出交易），再逐塊擴充至完整功能（帳本 CRUD、成員、分類、付款方式、交易分頁 / 篩選）。前端動工前先補一個範圍受控的後端小步（2a：付款方式 `PaymentMethod`，比照分類；見 `docs/specs/phase-2a-payment-methods.md`），使前端一開始即含付款方式、不返工。先只做 Web，行動 App（RN）待 Web 定型後再搬。
