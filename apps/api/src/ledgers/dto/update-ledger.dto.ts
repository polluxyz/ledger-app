 ...
 * `tracksBalance`（客戶端本就不該送），但這裡明確接住它，好回一句說得出原因的
 * 400。若不宣告，`forbidNonWhitelisted` 也會擋下來，但訊息只會說「不允許這個
 * 屬性」——看的人分不出是自己打錯字，還是這個欄位真的不能改。這樣寫也讓
 * 「建立後不可變更」這條規則出現在自動產生的 OpenAPI 文件上。
 */
